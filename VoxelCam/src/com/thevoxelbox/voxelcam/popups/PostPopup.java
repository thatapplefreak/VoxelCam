...
public class PostPopup extends GuiDialogBox {
		...
		poster.start(new ImgurCallback() {
			...
			public void onCompleted(ImgurResponse response) {
				...
				if (uploadResponse.isSuccessful()) {
					PostPopup.this.completeDialog = new UploadSuccessPopup(PostPopup.this.parentScreen, "Upload to imgur succeeded", uploadResponse.getDeleteHash(), uploadResponse.getLink());
				} else {
					PostPopup.this.completeDialog = new UploadFailedPopup(PostPopup.this.parentScreen, "Upload to imgur failed", uploadResponse.get("data"));
				}
			}
		});
		uploading = true;
	...
	private void doDropBox() {
		File src = GuiScreenShotManager.getScreenShotFiles().get(GuiScreenShotManager.selected);
		File dropboxCopy = new File(System.getProperty("user.home") + "/dropbox/mcScreenshots/", src.getName());
